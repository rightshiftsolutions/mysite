...
function normalizeQuestion(question, index) {
  return {
    ...question,
    // originalNo = the real question number from the MCQ file (never changes)
    originalNo: question.no || question.id || index + 1,
...
function getGameUiConfig(gameType) {
  const configs = {
    // Fix #6: Rapid Fire = +2 correct, 0 wrong, 5-min timer
    'RAPID_FIRE':       { icon: '⚡', title: 'Rapid Fire!', tagline: '+2 pts per correct answer — 5 minutes on the clock!', heroClass: 'game-hero-rapid', timerSeconds: 300, questionMessage: 'Think fast! ⚡', scoringInfo: '+2 pts correct · No penalty' },
    // Fix #5/Bonus: +3 correct
    'BONUS_POINTS':     { icon: '💎', title: 'Bonus Blast', tagline: '+3 bonus points for every correct answer!', heroClass: 'game-hero-bonus', timerSeconds: 0, questionMessage: 'Bonus time! 💎', scoringInfo: '+3 pts correct · No penalty' },
    // Fix #7: Negative Marking = +1 correct, -1 wrong
    'NEGATIVE_MARKING': { icon: '⚠️', title: 'Danger Zone', tagline: '+1 for correct, -1 for wrong — choose wisely!', heroClass: 'game-hero-negative', timerSeconds: 0, questionMessage: 'Stay careful! ⚠️', scoringInfo: '+1 pts correct · -1 pts wrong' },
    // Fix #8: No Negative = +2 correct, 0 wrong
    'NO_NEGATIVE_MARKING': { icon: '✅', title: 'Safe Zone', tagline: '+2 for correct answers, no penalty for wrong!', heroClass: 'game-hero-bonus', timerSeconds: 0, questionMessage: 'No risk! ✅', scoringInfo: '+2 pts correct · No penalty' },
    'SHADOWMIND': { icon: '🎓', title: 'KBC Arena', tagline: 'Kaun Banega Champion — answer wisely!', heroClass: 'game-hero-shadow', timerSeconds: 30, questionMessage: 'Think wisely! 🧠', scoringInfo: '+3 pts correct · Elimination on wrong' },
  };
...
function gameTypeLabel(type) {
  return { 'RAPID_FIRE': '⚡ Rapid Fire', 'BONUS_POINTS': '💎 Bonus','SHADOWMIND': '🎓 KBC', 'NEGATIVE_MARKING': '⚠️ Danger' }[type] || '🎮 Game';
}
...
async function loadMcq() {
  ...
  // Assign sequential display numbers (1, 2, 3…) AFTER shuffle.
  // question.no  → original MCQ number (used as answer key, never changes)
  // question.displayNo → sequential position in this session (1, 2, 3…)
...
// ── Intro Screen ───────────────────────────────────────────
function getRulesForGameType(gameType) {
  const rules = {
    // RAPID_FIRE and BONUS_POINTS rules removed per requirements
    'NEGATIVE_MARKING': {
      color: '#ff4455',
      icon: '⚠️',
      title: 'Negative Marking Rules',
      items: [
        { icon: '✅', label: '1 Right Question', value: '+1 Point',   highlight: true },
        { icon: '❌', label: '1 Wrong Question', value: '−1 Point',   highlight: true },
        { icon: '⏭️', label: 'Skipped Question', value: 'No Penalty', highlight: false },
      ],
      warning: 'Accuracy matters — every wrong answer deducts a point!'
    },
    'NO_NEGATIVE_MARKING': {
      color: '#39ff77',
      icon: '✅',
      ...
      items: [
        { icon: '✅', label: '1 Right Question', value: '+2 Points',  highlight: true },
        { icon: '❌', label: '1 Wrong Question', value: 'No Penalty', highlight: false },
        { icon: '⏭️', label: 'Skipped Question', value: 'No Penalty', highlight: false },
      ],
      warning: 'Attempt freely — no penalty for wrong answers!'
    },
  };
  return rules[gameType] || null;
}
...
function renderIntro() {
  ...
  const gameRules = getRulesForGameType(currentGame.gameType);
  const rulesHtml = gameRules ? `
    <div class="game-rules-panel" style="
      background: rgba(255,255,255,0.04);
      border: 1.5px solid ${gameRules.color}44;
      border-radius: 16px;
    ...
    ">
      <div style="
        ...
        gap:0.5rem;
      ">
        ${gameRules.icon} ${escapeHtml(gameRules.title)}
      </div>
      <div style="display:flex;flex-direction:column;gap:0.5rem;">
        ${gameRules.items.map(item => `
          <div style="
            display:flex;
            align-items:center;
            ...
            border-radius:8px;
            background:${item.highlight ? `${gameRules.color}15` : 'transparent'};
            border:1px solid ${item.highlight ? `${gameRules.color}33` : 'transparent'};
          ">
            <span style="color:var(--text-muted);font-size:0.88rem;">${item.icon} ${escapeHtml(item.label)}</span>
            <strong style="color:${item.highlight ? gameRules.color : 'var(--text-primary)'};font-size:0.92rem;">
      ...
      <div style="
        ...
        gap:0.4rem;
      ">
        💡 ${escapeHtml(gameRules.warning)}
      </div>
    </div>
  ` : (ui.scoringInfo ? `<div class="intro-scoring-info">🎯 Scoring: ${escapeHtml(ui.scoringInfo)}</div>` : '');
...
function beginGame() {
  ...
  // Task 3: Show KBC-style sticky timer for timed games (Rapid Fire)
...
function updateTimerBox() {
  ...
  const totalSec = 300; // Fix #4: always 5 minutes for Rapid Fire
...
function renderQuestion() {
  ...
  document.getElementById('questionHolder').innerHTML = `
    <div class="question-card-outer">
      <div class="d-flex justify-content-between align-items-center mb-3">
        <span class="question-number-tag">Question ${escapeHtml(String(question.displayNo || currentIndex + 1))}</span>
  ...
  nextBtn.textContent = currentIndex === questions.length - 1 ? '🏁 Submit' : 'Skip →';
}
...
function renderAnsweredPills() {
  ...
  // ── Stable container: fixed width, no reflow on state changes ──
  const totalQ = questions.length;
  const pillW  = 28; // px per pill
  const gap    = 4;  // gap between pills
  const totalW = totalQ * (pillW + gap);
  holder.style.width    = totalW + 'px';
  holder.style.minWidth = totalW + 'px';
  ...
  holder.innerHTML = questions.map((q, i) => {
    ...
    return `<span class="q-pill ${cls}"
      style="width:${pillW}px;height:${pillW}px;min-width:${pillW}px;box-sizing:border-box;display:inline-flex;align-items:center;justify-content:center;flex-shrink:0;"
...
function buildAnswerString() {
  // Answer string format: {questionNo}{optionId} concatenated, e.g. "1A3C7B"
  // Rules:
  //   - Use q.no (the ORIGINAL question number from the MCQ file) — never the display index
  //   - Skipped questions are simply OMITTED (backend treats missing key as skipped)
  //   - Including "X" for skips breaks parseAnswerString's [A-D] format check → 500 error
  let str = '';
  ...
  questions.forEach((q) => {
    ...
    // skipped → omit entirely; parseAnswerString returns {} for missing keys = treated as skipped
  });
...
async function submitGame(autoSubmit = false) {
  ...
  try {
    ...
    const data = await apiRequest('/api/student/submit-attempt', {
      method: 'POST',
      body: { attemptId: currentGame.id || currentGame.attemptId, answerString },
    });
    ...
    return `
      <div class="review-item review-${statusClass}">
        <div class="review-item-head">
          <span class="review-question-no">Q${escapeHtml(String(question.displayNo || question.no))}</span>
...
async function submitShadowMindAttempt(autoSubmit = false) {
  // ... other validation or loading animations ...

  // Call the robust function defined in game.js directly
  const answerString = buildAnswerString(); 

  const data = await apiRequest('/api/student/submit-attempt', {
    method: 'POST',
    body: { 
      attemptId: currentGame.id || currentGame.attemptId, 
      answerString 
    },
  });
  
  // ... rest of the submit handlers ...
}
