...
export async function setupCreateGameForm() {
  ...
  const courseSelect = document.getElementById('course-id');
  const streamHiddenInput = document.getElementById('stream-id');
  const streamDisplayInput = document.getElementById('stream-id-display');
  ...
  // Auto-detect the Stream whenever the Course selection changes.
  // The teacher no longer picks a stream manually — it's resolved
  // from the course's mapped (admin-approved) stream automatically.
  const autoDetectStream = async () => {
    const courseId = courseSelect.value;
    if (streamHiddenInput) streamHiddenInput.value = '';
    if (streamDisplayInput) streamDisplayInput.value = '';

    if (!courseId) return;

    try {
      streamDisplayInput.value = 'Detecting stream...';
      const res = await api.getCourseStreams(courseId);
      if (res.success && res.data && res.data.length > 0) {
        const stream = res.data[0];
        streamHiddenInput.value = stream.stream_id;
        streamDisplayInput.value = stream.stream_name;
      } else {
        streamDisplayInput.value = 'No approved stream linked to this course yet.';
      }
    } catch (error) {
  ...
  if (nextBtn) {
    nextBtn.addEventListener('click', () => {
      if (currentStep === 1) {
        ...
        if (!streamHiddenInput.value) {
          toast.warning('No admin-approved stream is linked to this course yet. Please select a different course or wait for stream approval.');
          return;
        }
      }
