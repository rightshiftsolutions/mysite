...
export async function loadStudentDashboard() {
  ...
  try {
    ...
    if (attendanceRes.success && attendanceRes.data) {
      ...
      if (el) {
        const todayRecords = attendanceRes.data.filter(r => {
          if (!r.attendance_date) return false;
          let dateStr = "";
          // Check if it's already a simple YYYY-MM-DD string
          if (typeof r.attendance_date === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(r.attendance_date)) {
            dateStr = r.attendance_date;
          } else {
            // Otherwise parse to local Date to respect client's timezone offset
        ...
        el.textContent = todayRecords.length;
      }
