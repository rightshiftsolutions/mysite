...
function renderPendingList(list) {
  ...
  const approveAllBtn = document.getElementById('btn-approve-all-attendance');
  ...
  if (role === 'teacher' && document.getElementById('pending-table-body')) {
    ...
    if (approveAllBtn) {
      approveAllBtn.addEventListener('click', async () => {
        try {
          loader.show('Approving all pending attendances...');
          const res = await api.approveAllAttendance();
          loader.hide();
          if (res.success) {
            toast.success(res.message || 'Approved all requests successfully.');
            loadPendingAttendance();
          }
        } catch (error) {
          loader.hide();
