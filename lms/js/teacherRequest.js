...
export async function loadTeacherRequests() {
  const tbody = document.getElementById('teacher-requests-body');
  if (!tbody) return;

  try {
    loader.show('Loading pending requests...');
...
function adjustRequestContainersLayout() {
  const teacherContainer = document.getElementById('teacher-requests-container');
  const studentContainer = document.getElementById('student-requests-container');
  const rowContainer = document.getElementById('requests-row-container');
  if (!teacherContainer || !studentContainer) return;

  const teacherHidden = teacherContainer.classList.contains('d-none');
  const studentHidden = studentContainer.classList.contains('d-none');

  if (teacherHidden && studentHidden) {
    if (rowContainer) rowContainer.classList.add('d-none');
  } else {
    if (rowContainer) rowContainer.classList.remove('d-none');
    
    if (!teacherHidden && !studentHidden) {
      teacherContainer.classList.remove('col-12');
      teacherContainer.classList.add('col-md-6');
      studentContainer.classList.remove('col-12');
      studentContainer.classList.add('col-md-6');
    } else if (!teacherHidden) {
      teacherContainer.classList.remove('col-md-6');
      teacherContainer.classList.add('col-12');
    } else if (!studentHidden) {
      studentContainer.classList.remove('col-md-6');
      studentContainer.classList.add('col-12');
    }
  }
}
...
async function processStatusChange(id, name, status) {
  ...
  try {
    loader.show(`${statusLabel} teacher request...`);
    const res = await api.updateTeacherStatus(id, status);
    loader.hide();
...
export function setupRequestsSearch() {
  const searchInput = document.getElementById('teacher-requests-search-input') || document.getElementById('requests-search-input');
...
document.addEventListener('DOMContentLoaded', () => {
  if (document.getElementById('teacher-requests-body')) {
    loadTeacherRequests();
    setupRequestsSearch();
  }
});
