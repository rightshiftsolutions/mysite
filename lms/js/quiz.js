...
export async function initializeQuizRunner() {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Enable anti-cheating tab-switching / blur listener
...
// ==========================================
// Anti-cheating (Tab switching / Focus loss detection)
// ==========================================
let cheatingSubmitted = false;

function autoSubmitOnCheating() {
  if (cheatingSubmitted) return;
  if (!attemptId || eliminated) return;

  cheatingSubmitted = true;
  removeAntiCheatingListeners();

  toast.warning('Auto-submitting quiz because you left the browser tab/window.');
  submitQuizGame(true);
}
              ...
              return `
                    <div class="mb-3">
                      <div class="d-flex justify-content-between small fw-bold mb-1">
                        <span>Option ${lbl}: ${question.options[lbl.charCodeAt(0) - 65]}</span>
                        <span>${pct}%</span>
                      </div>
