    ...
    if (res.success && res.data) {
      ...
      container.innerHTML = `
        <div class="result-card-custom text-center bg-white rounded-4 border p-5">
          ...
          <div class="result-circular-progress" id="result-circular" style="--progress-value: 0; flex-direction: column;">
            <div class="result-circular-text" id="result-percentage-text" style="line-height: 1.1;">0%</div>
            <div class="text-white-50 mt-1" id="result-fraction-text" style="font-size: 0.85rem; font-weight: 600;">0/0</div>
          </div>
            ...
            <div class="col-md-4">
              <div class="performance-stat-item shadow-premium border-success-subtle">
                <div>
                  ...
                  <div class="fw-bold text-success fs-5"><i class="bi bi-check-circle-fill me-1.5"></i>${correctAnswers} / ${totalQuestions}</div>
                </div>
              </div>
            </div>
                ...
                return `
                  <div class="card border rounded-3 p-3 mb-3 shadow-sm">
                    <div class="d-flex justify-content-between align-items-start gap-2 mb-2">
                      <span class="fw-bold text-dark">Q${idx + 1}. ${escapeHtml(q.question)}</span>
      ...
      animateResultReveal(percentage, data.score, Number(data.rank), correctAnswers, totalQuestions);
    }
  } catch (error) {
    loader.hide();
...
// ==========================================
function animateResultReveal(percentage, score, rank, correct, total) {
  ...
  const fractionText = document.getElementById('result-fraction-text');
  if (!ring || !pctText || !scoreText) return;

  if (fractionText) {
    fractionText.textContent = `${correct}/${total}`;
  }
