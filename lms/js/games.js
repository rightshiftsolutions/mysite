    ...
    btn.addEventListener('click', async (e) => {
      ...
      try {
        ...
        if (res.success) {
          ...
          loadTeacherGamesTable(activeFilters);
        }
      } catch (err) {
        loader.hide();
...
function renderStudentGames(list, containerId = 'student-games-container', options = {}) {
  ...
  container.innerHTML = list.map(game => {
    ...
    ` : `
        <div class="d-flex gap-2">
          ...
          <button class="btn btn-sm btn-primary rounded-pill px-3 flex-fill start-student-game-btn" data-id="${game.game_id}" data-time="${game.time_limit}" data-title="${game.game_name}">
            Start Game
          </button>
        </div>
    `;
  ...
  container.querySelectorAll('.start-student-game-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      confirmStartQuiz(id, time, title);
    });
  });
}

function confirmStartQuiz(gameId, timeLimitSeconds, gameTitle) {
  import('../components/modal.js').then(({ modal }) => {
    modal.show({
      ...
      onConfirm: async () => {
        try {
          ...
          if (res.success && res.data) {
            ...
            localStorage.setItem('active_game_title', gameTitle || 'Quiz Game');
  ...
  container.querySelectorAll('.page-link').forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      if (page >= 1 && page <= totalPages) {
        currentPage = page;
        if (role === 'teacher') loadTeacherGamesTable(activeFilters);
        else loadStudentGames(activeFilters);
      }
    });
  });
}
...
export async function populateGameFilters() {
  ...
  try {
    const promises = [api.getMyCourses().catch(() => ({ success: false, data: [] }))];
    if (streamSelect) {
      promises.push(api.getMyStreams().catch(() => ({ success: false, data: [] })));
    }

    const results = await Promise.all(promises);
    const coursesRes = results[0];
    const streamsRes = results[1];
    ...
    if (streamSelect && streamsRes && streamsRes.success) {
...
export function setupGameFilters() {
  ...
  if (!courseSelect && !unitInput) return;
  ...
  const runFilters = () => {
    ...
    if (role === 'teacher') loadTeacherGamesTable(filters);
    else loadStudentGames(filters);
  };

  let debounceTimer = null;
  const debouncedFilters = () => {
    clearTimeout(debounceTimer);
    debounceTimer = setTimeout(runFilters, 300);
  };

  if (courseSelect) {
    courseSelect.addEventListener('change', runFilters);
  }

  if (unitInput) {
    unitInput.addEventListener('input', debouncedFilters);
  }
}
