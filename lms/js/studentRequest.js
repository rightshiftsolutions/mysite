...
function renderRequests(list) {
  const tbody = document.getElementById('student-requests-body');
  const approveAllBtn = document.getElementById('btn-approve-all-students');
  if (!tbody) return;
  ...
  if (document.getElementById('student-requests-body')) {
    ...
    if (approveAllBtn) {
      approveAllBtn.addEventListener('click', async () => {
        try {
          loader.show('Approving all student signup requests...');
          const res = await api.approveAllStudentRequests();
          loader.hide();
          if (res.success) {
            toast.success(res.message || 'All student requests approved successfully.');
            loadStudentRequests();
          }
        } catch (error) {
          loader.hide();
          toast.error(error.message || 'Failed to approve student requests.');
        }
      });
    }
  }
